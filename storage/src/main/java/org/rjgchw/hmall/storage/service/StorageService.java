...
public class StorageService {
     ...
     * @return 成功/失败
     */
    public Optional<StorageDeductDTO> lockStorage(Long productId, Integer productQuantity) {
        return storageRepository.findByProductId(productId)
            .map(x -> {
                if(x.getStorage() < productQuantity) {
                    throw new StorageShortageException();
                }
                return Optional.of(new StorageDeductDTO(
                    productId,
                    productQuantity,
                    storageRepository.lockProduct(productId, x.getStorage(), productQuantity) > 0
                ));
            })
